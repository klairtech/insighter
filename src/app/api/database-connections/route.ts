...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!minimumCreditsCheck.hasCredits) {
      console.log(`❌ Database Connections API: User has insufficient credits for adding data sources. User has ${minimumCreditsCheck.currentCredits}, needs minimum 10 credits`)
      return NextResponse.json(
        { 
